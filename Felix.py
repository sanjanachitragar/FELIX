...
# Set the desired voice index (change the index according to the voice you want)
...
if __name__ == '__main__':
    ...
    while True: 
        ...
        if 'wikipedia' in query:
           ...
           file.close
            ...
            fun_talk("I don't know Alexa, but I've heard of Alexa. If you have Alexa, "
                        "I may have just triggered Alexa. If so, sorry AFlexa.")
        ...
        elif 'exit felix' in query or 'exit felix' in query:
            fun_talk("Exiting ...")
            sys.exit()
            ...
            res=ravness_model.model_predict()
            def suggest_movie(mood):
                if mood == 'calm':
                    # Suggest calm movies (e.g., documentaries)
                    calm_movies = ['The Social Dilemma', 'Matrix', 'American Sniper']
                    return random.choice(calm_movies)
                elif mood == 'neutral':
                    # Suggest neutral movies (e.g., thrillers)
                    neutral_movies = ['Wednesday', 'World War Z', 'Train to Busan']
                    return random.choice(neutral_movies)
                elif mood == 'sad' or mood ==['male_angry'] or mood ==['female_angry'] :
                    # Suggest sad movies (e.g., dramas)
                    sad_movies = ['The Pursuit of Happyness', 'Yeh Jawaani Hai Deewani', 'Jungle Cruise']
                    return random.choice(sad_movies)
                elif mood == 'happy':
                    # Suggest happy movies (e.g., comedies, romances)
                    happy_movies = ["It's Okay to Not Be Okay", '3 Idiots', 'When Harry Met Sally']
                    return random.choice(happy_movies)
                else:
                    # Return a default suggestion if mood is not recognized
                    return "Sorry, I couldn't find any movie suggestion for that mood."
            
              
            suggested_movie = suggest_movie(res)
            print("Recommended Movie:", suggested_movie)
            ...
            url = f"https://www.google.com/maps/dir/{location1}/{location2}"
            webbrowser.open(url)

            # Register the Google Maps app as the browser to use
            #webbrowser.register('googlemaps', None, webbrowser.BackgroundBrowser("C:\Program Files\Google\Chrome\Application\chrome.exe"))
            #webbrowser.get('googlemaps').open(url)
